set checkpoint "/home/fy2243/soc_design/pd/latchfix_20260422_r2_postfill/postfill_cleaned.enc.dat"
set design     "soc_top"
set out_root   "/home/fy2243/soc_design/pd/latchfix_20260422_r3_sram_hold"
set sta_root   "/home/fy2243/soc_design/sta/currentrtl_latchfix_20260422_r3_sram_hold/innovus"
set view       "view_typ"
...
proc safe_run {label cmd} {
    puts "R3_STEP_BEGIN $label"
    set rc [catch {uplevel 1 $cmd} msg]
    puts "R3_STEP_END $label rc=$rc msg=$msg"
    return [list $rc $msg]
}
...
set hold_terms {}
append_bus_terms hold_terms "u_sram/u_sram_macro/D" 0 31
append_bus_terms hold_terms "u_sram/u_sram_macro/A" 0 8
append_bus_terms hold_terms "u_sram/u_sram_macro/BWEB" 0 31

set inserted 0
set failed {}
set index 0
foreach term $hold_terms {
    set inst_name [format "PT_HOLD_R3_%03d" $index]
    set net_name  [format "PT_HOLD_R3_%03d_net" $index]
    set rc [catch {
        ecoAddRepeater -term $term -cell $delay_cell -name $inst_name -newNetName $net_name -relativeDistToSink 0.10
    } msg]
    puts "R3_HOLD_ECO index=$index term=$term inst=$inst_name cell=$delay_cell rc=$rc msg=$msg"
    if {$rc == 0} {
        incr inserted
    } else {
...
set checkpoint_out [file join $out_root postfill_sram_hold.enc]
saveDesign $checkpoint_out
saveNetlist [file join $sta_root ${design}_postroute_latchfix_r3_sram_hold.v]
rcOut -spef [file join $sta_root ${design}_postroute_latchfix_r3_sram_hold.spef] -view $view -cUnit pF
write_sdf [file join $sta_root ${design}_postroute_latchfix_r3_sram_hold.sdf] -view $view -recompute_delay_calc -precision 4

set fp [open [file join $sta_root summary.txt] w]
puts $fp "Latchfix r3 SRAM-interface targeted hold ECO summary"

...
set pnr_out_dir [expr {[info exists ::env(SOC_PNR_OUT_DIR)] && $::env(SOC_PNR_OUT_DIR) ne "" ? [file normalize $::env(SOC_PNR_OUT_DIR)] : [file join $proj_root pd postroute_corepin_reroute_fast]}]
...
puts "COREPIN_REROUTE_BASELINE DRC=[parse_drc_violations $pre_drc] REG=[parse_connectivity_total $pre_reg] SPC=[parse_connectivity_total $pre_spc]"
...
saveDesign [file join $pnr_out_dir with_sram_corepin_reroute.enc]
puts "COREPIN_REROUTE_SUMMARY DRC=[parse_drc_violations $drc_rpt] REG=[parse_connectivity_total $reg_rpt] SPC=[parse_connectivity_total $spc_rpt]"
exit

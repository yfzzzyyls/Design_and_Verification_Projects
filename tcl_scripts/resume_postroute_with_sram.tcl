# Resume the SRAM flow from a saved routed checkpoint and finish fill/signoff gates.
...
set route_src_dir [expr {[info exists ::env(SOC_ROUTE_SRC_DIR)] && $::env(SOC_ROUTE_SRC_DIR) ne "" ? [file normalize $::env(SOC_ROUTE_SRC_DIR)] : $pnr_out_dir}]
set route_ckpt [file join $route_src_dir with_sram_route.enc.dat]
set skip_metalfill [expr {[info exists ::env(SOC_SKIP_METALFILL)] && $::env(SOC_SKIP_METALFILL) ne "" && $::env(SOC_SKIP_METALFILL) ne "0"}]
...
proc parse_drc_violations {rpt} {
    ...
    if {[regexp -nocase {Verification\s+Complete\s*:\s*([0-9]+)\s+Viols?} $content -> num]} {
        return $num
    }
    return -1
}
...
proc soc_get_design_bbox {} {
    ...
    if {[llength $bbox] != 4} {
        fail_flow 11 "Unexpected design bbox format: $bbox_raw"
    }
    return $bbox
}
...
if {![file exists $route_ckpt]} {
    fail_flow 10 "Missing route checkpoint: $route_ckpt"
}

restoreDesign $route_ckpt soc_top

# Targeted post-route replay for the stubborn SRAM-side VDD hotspot.
# It reproduces the post-fill state, lets ecoRoute collapse regular filler
# issues, then trims the residual M4 VDD special shapes near the SRAM edge and
# checks whether native DRC/connectivity become clean.

set script_dir [file dirname [file normalize [info script]]]
set proj_root  [file normalize [file join $script_dir ..]]
set pnr_out_dir [expr {[info exists ::env(SOC_PNR_OUT_DIR)] && $::env(SOC_PNR_OUT_DIR) ne "" ? [file normalize $::env(SOC_PNR_OUT_DIR)] : [file join $proj_root pd postroute_trim_sram_vdd_hotspot]}]
...
proc trim_sram_vdd_hotspot {} {
    set hotspot_box {104.800 60.900 105.500 65.000}
    deselectAll
    editSelect -net VDD -type Special -layer M4 -area $hotspot_box
    set sel_count [llength [dbGet selected]]
    puts "Hotspot selection count: $sel_count"
    if {$sel_count == 0} {
        fail_flow 40 "No VDD M4 special shapes selected in hotspot box."
    }
    catch {puts "Hotspot boxes: [dbGet selected.box]"}
    editDelete -selected
    deselectAll
}

puts "\n=========================================="
puts "POST-ROUTE TRIM SRAM VDD HOTSPOT"
puts "==========================================\n"
...
if {$drc2 != 0} {
    fail_flow 42 "Hotspot trim did not clear DRC (remaining $drc2)."
}
...
if {$regular_errors != 0} {
    fail_flow 43 "Regular connectivity failed after hotspot trim ($regular_errors)."
}
...
if {$special_errors != 0} {
    fail_flow 44 "Special connectivity failed after hotspot trim ($special_errors)."
}

saveDesign [file join $pnr_out_dir with_sram_final.enc]
puts "\n=========================================="
puts "HOTSPOT TRIM RESULT: PASS"
puts "==========================================\n"
exit

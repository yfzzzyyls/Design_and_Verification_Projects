# Resume from a saved place checkpoint, apply the current SRAM-aware PG route
# policy, then finish CTS/route/fill/signoff gates.

set script_dir [file dirname [file normalize [info script]]]
set proj_root  [file normalize [file join $script_dir ..]]
set pnr_out_dir [expr {[info exists ::env(SOC_PNR_OUT_DIR)] && $::env(SOC_PNR_OUT_DIR) ne "" ? [file normalize $::env(SOC_PNR_OUT_DIR)] : [file join $proj_root pd innovus_resume_from_place_with_sram_pgfix]}]
set place_enc [expr {[info exists ::env(SOC_PLACE_ENC)] && $::env(SOC_PLACE_ENC) ne "" ? [file normalize $::env(SOC_PLACE_ENC)] : [file join $proj_root pd innovus with_sram_place.enc.dat]}]
...
puts "\n=========================================="
puts "RESUME FROM PLACE WITH SRAM PG FIX"
puts "==========================================\n"
puts "Place checkpoint: $place_enc"

if {![file exists $place_enc]} {
    fail_flow 10 "Missing place checkpoint: $place_enc"
}

restoreDesign $place_enc soc_top
...
puts "\n=========================================="
puts "RESUME FROM PLACE RESULT: PASS"
puts "==========================================\n"
exit

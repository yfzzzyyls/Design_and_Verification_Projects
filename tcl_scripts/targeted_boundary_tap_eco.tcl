set route_enc [expr {[info exists ::env(SOC_ROUTE_ENC)] && $::env(SOC_ROUTE_ENC) ne "" ? [file normalize $::env(SOC_ROUTE_ENC)] : "/home/fy2243/soc_design/pd/innovus_foundrytap_20260410/with_sram_route.enc.dat"}]
set pnr_out_dir [expr {[info exists ::env(SOC_PNR_OUT_DIR)] && $::env(SOC_PNR_OUT_DIR) ne "" ? [file normalize $::env(SOC_PNR_OUT_DIR)] : "/home/fy2243/soc_design/pd/innovus_targetedtap_20260410"}]
...
# Replace specific boundary-row filler segments with extra boundary taps.
foreach {inst_name cell_name x y orient delete_pattern} {
    ECO_BOT_TAP_A BOUNDARY_NTAPBWP16P90LVT_VPP_VSS 100.800 50.016 MX BOUNDARY_NROW*
    ECO_BOT_TAP_B BOUNDARY_NTAPBWP16P90LVT_VPP_VSS 202.320 50.016 MX BOUNDARY_NROW*
    ECO_TOP_TAP_A BOUNDARY_NTAPBWP16P90LVT_VPP_VSS 100.800 285.600 R0 BOUNDARY_NROW*
    ECO_TOP_TAP_B BOUNDARY_NTAPBWP16P90LVT_VPP_VSS 202.320 285.600 R0 BOUNDARY_NROW*
} {
    delete_matching_in_box $x $y [expr {$x + 0.72}] [expr {$y + 0.576}] $delete_pattern
    add_fixed_inst $inst_name $cell_name $x $y $orient
}

# Fill the missing bottom regular-tap slot on the right-side strip.
add_fixed_inst ECO_BOT_ROW1_TAP TAPCELLBWP16P90LVT_VPP_VSS 253.080 50.592 R0
...
puts "TARGETED_TAP_ECO_SUMMARY DRC=[parse_drc_violations $drc_rpt] REG=[parse_connectivity_errors $conn_regular_rpt] SPC=[parse_connectivity_errors $conn_special_rpt] ANT=[parse_antenna_violations $antenna_rpt]"
exit

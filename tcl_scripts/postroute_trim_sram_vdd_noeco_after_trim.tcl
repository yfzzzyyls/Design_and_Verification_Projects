...
set pnr_out_dir [expr {[info exists ::env(SOC_PNR_OUT_DIR)] && $::env(SOC_PNR_OUT_DIR) ne "" ? [file normalize $::env(SOC_PNR_OUT_DIR)] : [file join $proj_root pd postroute_trim_sram_vdd_noeco_after_trim]}]
...
proc trim_duplicate_m4_shapes {} {
    ...
    foreach box $delete_boxes {
        ...
        puts "Delete box $box selected [llength [dbGet selected]] object(s)"
        if {[llength [dbGet selected]] > 0} {
            editDelete -selected
        }
    }
    deselectAll
...
puts "\n=========================================="
puts "POST-ROUTE TRIM SRAM VDD NO ECO AFTER TRIM"
puts "==========================================\n"
...
puts "Iter1 DRC: [parse_drc_violations $rpt1]"

trim_duplicate_m4_shapes

set rpt2 [file join $pnr_out_dir drc_after_delete_noeco.rpt]
verify_drc -limit 10000 -report $rpt2
puts "After-delete-noeco DRC: [parse_drc_violations $rpt2]"

set reg_rpt [file join $pnr_out_dir lvs_connectivity_regular_noeco.rpt]
verifyConnectivity -type regular -error 1000 -warning 100 -report $reg_rpt
puts "After-delete-noeco regular: [parse_connectivity_errors $reg_rpt]"

set sp_rpt [file join $pnr_out_dir lvs_connectivity_special_noeco.rpt]
verifyConnectivity -type special -noAntenna -error 1000 -warning 100 -report $sp_rpt
puts "After-delete-noeco special: [parse_connectivity_errors $sp_rpt]"

saveDesign [file join $pnr_out_dir with_sram_after_delete_noeco.enc]
exit

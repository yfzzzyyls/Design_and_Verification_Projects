...
set design     "soc_top"
set out_root   "/home/fy2243/soc_design/pd/holdfix_currentrtl_20260418"
set sta_root   "/home/fy2243/soc_design/sta/currentrtl_20260418_holdfix/innovus"
set view       "view_typ"
...
puts ""
puts "=========================================="
puts "CurrentRTL Post-Route Hold ECO"
puts "=========================================="
puts "Checkpoint: $checkpoint"
puts "Output Dir: $out_root"
puts "STA Dir:    $sta_root"
puts "=========================================="
puts ""

# Keep analysis consistent with the routed-signoff probe.
setExtractRCMode -engine postRoute -effortLevel medium
catch {extractRC}
setAnalysisMode -analysisType onChipVariation -cppr both

# Bias hold fixing toward small delay cells/buffers. Setup has large margin, but
# do not allow setup TNS degradation while repairing hold.
...
# Re-extract and remeasure after hold ECO.
catch {extractRC}
...
saveDesign [file join $out_root with_sram_holdfix.enc]
saveNetlist [file join $sta_root ${design}_postroute_holdfix.v]
rcOut -spef [file join $sta_root ${design}_postroute_holdfix.spef] -view $view -cUnit pF
write_sdf [file join $sta_root ${design}_postroute_holdfix.sdf] -view $view -recompute_delay_calc -precision 4
...
puts $fp "checkpoint_out=[file join $out_root with_sram_holdfix.enc]"
